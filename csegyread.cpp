...
{
/*
这是 C/C++ 语言中标准库函数 `fseek()` 的调用，用于将文件指针移动到指定的文件位置。

具体地，`m_fpr` 是一个文件指针（通常是 `FILE*` 类型），通过该指针可以对文件进行读写操作。`fseek()`
函数的第一个参数为文件指针，表示要操作的文件；第二个参数为偏移量，表示要将文件指针向后或向前移动多少个字节；
第三个参数为起始位置，可选值包括 `SEEK_SET`（从文件头开始计算偏移量）、`SEEK_CUR`（从当前位置开始计算偏移量）
和 `SEEK_END`（从文件尾开始计算偏移量）。
在这个例子中，`fseek(m_fpr,0,0)` 表示将文件指针 `m_fpr` 移动到文件开头，即偏移量为 0 字节，起始位置为文件
开头（`SEEK_SET`）。这样一来，之后的文件读写操作就会从文件开头开始。
*/
    fseek(m_fpr,0,0); // 初始m_fpr = NULL, FILE类型
    if (m_bHasTextualHeader)//判断含文本卷头信息
    {
...
/*在使用fread函数时，我们需要指定读取的字节数和数据类型，而fread函数不支持直接读取float类型数据。实际上，fread函数只能以字节为单位进行读取，并没有内置的数据类型转换功能。

因此，在读取float类型数据时，需要先将读取到的字节流按照float类型进行解析，这个过程就需要手动处理。*/
...
float* CSegyRead::GetTraceData(int nIndex )
{
    ...
    // m_pData = new float[m_nSampleNum];
    m_pData = (float*)alloca(m_nSampleNum * sizeof(float));
    ...
    float *m_pData = new float[len];// ---tmp 存在内存释放问题，因此使用new
    // float *m_pData = (float*)alloca(len * sizeof(float));
    // pdata = (float*)alloca(len * sizeof(float));
...
bool CSegyRead::OpenFile(const char *csReadFile)
{
...
{
    m_nFormatCode = m_volHead.format; //设置数据类型
